	...
	"A Tui" when {
		"initializing" should {
			...
			"throw exception when scale < 3" in {
				an[IllegalArgumentException] should be thrownBy new Tui(new Controller(Board(), RulesFake(), playerGenerator = playerGenerator), 1, 1, 2, (0, 0))
			}
			"throw exception when width < 1" in {
				an[IllegalArgumentException] should be thrownBy new Tui(new Controller(Board(), RulesFake(), playerGenerator = playerGenerator), 0, 1, 3, (0, 0))
			}
			"throw exception when height < 1" in {
				an[IllegalArgumentException] should be thrownBy new Tui(new Controller(Board(), RulesFake(), playerGenerator = playerGenerator), 1, 0, 3, (0, 0))
			}
		}
		"initialized" should {
			...
			"set cursor to offset on 'cursor reset'" in {
				val tui = new Tui(new Controller(Board(), RulesFake(), playerGenerator = playerGenerator), width, height, scale, (1, 10))
