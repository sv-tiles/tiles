	...
	"A Tui" when {
		"initializing" should {
			...
			"throw exception when scale < 3" in {
				an[IllegalArgumentException] should be thrownBy new Tui(new Controller(Board(), RulesFake()), 1, 1, 2, (0, 0))
			}
			"throw exception when width < 1" in {
				an[IllegalArgumentException] should be thrownBy new Tui(new Controller(Board(), RulesFake()), 0, 1, 3, (0, 0))
			}
			"throw exception when height < 1" in {
				an[IllegalArgumentException] should be thrownBy new Tui(new Controller(Board(), RulesFake()), 1, 0, 3, (0, 0))
			}
		}
		"initialized" should {
			...
			"move offset (x/y) by 1 on 'w', 'a', 's', 'd'" in {
				val tui = new Tui(new Controller(Board(), RulesFake()), width, height, scale, (0, 0))
				val original = new Tui(new Controller(Board(), RulesFake()), width, height, scale, (0, 0))
			...
			"set cursor to offset on 'cursor reset'" in {
				val tui = new Tui(new Controller(Board(), RulesFake()), width, height, scale, (1, 10))
			...
			"rotate tiles on 'q' and 'r' counterclockwise" in {
				...
				val controller = new Controller(Board(new HashMap().updated(Position(0, 0), tile1), Option.empty, Option(Position(0, 0))), RulesFake())
			...
			"reset board on 'clear'" in {
				val controller = new Controller(Board(new HashMap().updated(Position(10, 10), tile).updated(Position(0, 0), tile)), RulesFake())
				val tui = new Tui(controller, width, height, scale, (0, 0))
